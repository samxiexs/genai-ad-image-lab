...
为方便实验设计，本项目现在把研究条件理解成四格结构，而不是三个并列理论条件。基础核是 `definition-only`，其余条件是在此基础上叠加 `visual control` 与 `generated prompt` 两个组件。

| 新研究条件 | 主要来源 | 运行形态 | 核心逻辑 |
| --- | --- | --- | --- |
| `definition-only` | 确认后的 `def-*` | 单阶段 | 理论定义直出 |
| `definition-control` | `def-*` + visual-control block | 单阶段 | `def + vc` |
| `definition-genprompt` | `def-*` + generator | 两步流程 | `def -> generated prompt -> image` |
| `definition-control-genprompt` | `def-*` + generator + `dc-*` wrapper | 两阶段 | `def -> generated prompt -> dc wrapper -> image` |

兼容版本名 `visual-control` / `genprompt-control` 仍然保留，但现在只是 `definition-control` / `definition-control-genprompt` 的兼容入口。
...
| 目标                                              | 建议版本         | 理由                                                                                   |
| ------------------------------------------------- | ---------------- | -------------------------------------------------------------------------------------- |
| 需要直接运行当前四格研究条件 | `definition-only` / `definition-control` / `definition-genprompt` / `definition-control-genprompt` | 便于把理论定义、visual control、generated-prompt decomposition 明确拆开写；旧版本名 `visual-control` / `genprompt-control` 仍可兼容调用 |
...
- `docs/research_prompt_conditions_v5_v8_v17.md`: 四格研究条件 `definition-only`、`definition-control`、`definition-genprompt`、`definition-control-genprompt` 的 evidence map，以及与 CCF controllable-generation 文献的对应说明。
