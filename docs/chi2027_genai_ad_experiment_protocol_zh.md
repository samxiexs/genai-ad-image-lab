...
Khatiwada 等人的 CHI 新闻研究先测量未披露来源时的内容评价，再披露 LLM 参与并引导反思 [R18]。本研究保留这个思路，但同时预先写入两条可选路线：

**路线 A：后曝光前后披露。** 参与者先在不知道 AI 来源的情况下评价 9 张图，再随机接受真实 AI 披露或等时中性反思，并重新评价同一组图片。它回答的是：**在已经形成初始印象后，补充 AI 来源信息是否改变评价？**

**路线 B：首次曝光直接分组。** 参与者在第一次看到图片之前，就被随机分到“AI 已披露”或“来源不披露”组，然后只评价一次 9 张图。它回答的是：**消费者从一开始就知道或不知道 AI 来源时，评价是否不同？**

两条路线不能事后根据显著性选择，也不应让同一参与者同时参加两条路线。正式预注册时选择其中一条作为确认性 Study 2；若经费允许，两条路线使用独立样本分别运行，并作为 Study 2A（后曝光披露）和 Study 2B（首次曝光分组）报告。两条路线的效应不能直接当成同一个因果 estimand 合并。

### 6.2 设计与样本量

两条路线都使用通过验证的 GenAI 广告图、同一组 12 个商品、同一套 9 图商品类型分配：`3（策略，组内）× 3（具体商品 exemplar，组内）× 2（商品取向，组间）× 2（象征性，组间）`。每名参与者随机进入一个商品类型格，评价该格全部 `3 商品 × 3 策略 = 9` 张图。两条路线的区别只在于 AI 来源信息出现的时点和是否有第二轮测量。

**路线 A：后曝光前后披露（推荐用于复制 R18 的时间逻辑）**

| 分配组别 | 每格有效样本 | 每位参与者做什么 |
| --- | ---: | --- |
| U-LS × AI 披露 | 30 | 9 张图披露前评价 → AI 披露 → 9 张图披露后评价 |
| U-LS × 中性反思 | 30 | 9 张图第一次评价 → 中性反思 → 同 9 张图第二次评价 |
| U-HS × AI 披露 | 30 | 同上 |
| U-HS × 中性反思 | 30 | 同上 |
| H-LS × AI 披露 | 30 | 同上 |
| H-LS × 中性反思 | 30 | 同上 |
...
| 分配组别 | 每格有效样本 | 每位参与者做什么 |
| --- | ---: | --- |
| U-LS × AI 已披露 | 30 | 先看 AI 来源说明，再评价该格 9 张图一次 |
| U-LS × 来源不披露 | 30 | 不显示来源信息，直接评价该格 9 张图一次 |
| U-HS × AI 已披露 | 30 | 同上 |
| U-HS × 来源不披露 | 30 | 同上 |
| H-LS × AI 已披露 | 30 | 同上 |
| H-LS × 来源不披露 | 30 | 同上 |
...
`N=240` 是每条路线各自的暂定有效样本，而不是两条路线合计 240 人。如果两条路线都实施，总样本为 `N=480`；如果预算只有 240 人，应在预注册前选择其中一条，而不能把 240 人拆成两条路线后仍声称有 30 人/格。每条路线的最终人数都要由 pilot 的重复测量相关性、披露效应大小和预期排除比例进行模拟功效确认；若主检验达不到预注册的 `80%` power，则增加每个商品格的人数。

### 6.3 完整处理文字

两条路线的页面都具有相同布局、最低阅读时间和“继续”按钮；不同之处只在于 AI 来源信息出现的时点。
...
> 接下来您将看到的九张广告图片均由生成式 AI 系统基于去品牌化的产品参考图和研究定义的视觉指令生成。它们是研究材料，并不构成商业产品承诺。

**路线 B：首次曝光来源不披露条件**

该组不显示任何 AI、人工制作或作者来源说明，只显示统一的任务说明：“接下来请逐张查看并评价广告图片。”这样可以比较“首次看到时已知 AI”与“首次看到时没有来源信息”，而不是比较 AI 与虚假人类来源。

所有参与者最终都在 debrief 中获知图像确由生成式 AI 制作；任何路线都不向参与者提供“由人类制作”的虚假信息。
...
`time × disclosure` 是 AI 标签相对“单纯重看/反思”的因果效应；`time × disclosure × strategy` 检验该效应是否因三种策略不同。`phase_position` 控制各阶段第 1--9 次图片展示；第二轮使用独立受约束顺序。主要结果为 D1/D2 的前后变化，并对 D1/D2 做 Holm 校正。

**路线 B：首次曝光直接分组比较**

1. 同 Study 1 完成同意与资格检查；先随机分配到商品类型格，再随机分配到“AI 已披露”或“来源不披露”条件。
2. AI 已披露组在第一张图片出现前看到 AI 来源说明；来源不披露组只看到统一任务说明。
3. 两组都用同一套受约束随机顺序逐张评价该格 9 张图；每张图后测 D1/D2 两题短版、C1 路线选择题和必要的图像级控制题；不进行第二轮评价。
4. 测 D3/D4、三个商品的 C3 熟悉度、C4、注意力与人口统计，随后真实 debrief。
...
路线 B 中，`disclosure` 是组间变量，`strategy` 是组内变量。`disclosure` 是“首次看到广告时是否已知 AI 来源”的直接组间效应；`disclosure × strategy` 检验 AI 来源提示是否对三种视觉策略产生不同影响。`presentation_position` 控制第 1--9 张图片的位置。D3 只作探索性过程证据，不进入主要结果回归。

路线 A 和路线 B 的效应不能直接合并为一个确认性系数：路线 A 估计的是“**初始评价后补充 AI 信息的改变**”，路线 B 估计的是“**首次接触前已知 AI 与未披露的差异**”。如果两条路线都实施，应分别报告，跨路线比较只作为探索性分析。
...
| AI 来源 | 路线 A 为后曝光真实披露/等时反思；路线 B 为首次曝光 AI 已披露/来源不披露 | Khatiwada 等 [R18]；Koning & Voorveld [R16] |
